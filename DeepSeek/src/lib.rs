...
/// Fixed count of fractional digits produced by legacy division before formatting.
pub const DIV_FRACTIONAL_DIGITS: usize = 20;
/// Legacy formatter prints at most this many fractional digits (with broken rounding).
pub const LEGACY_ROUND_DIGITS: usize = 10;
/// Legacy "too small" divisor check looks at this many leading digits.
pub const TOO_SMALL_PREFIX_DIGITS: usize = 7;

/// Sign of a decimal number.
...
/// Errors that can occur while parsing or computing with `BigDecimal`.
...
impl fmt::Display for DecimalError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            DecimalError::Empty => write!(f, "input is empty"),
            DecimalError::InvalidChar(ch) => write!(f, "invalid character: {}", ch),
            DecimalError::MultipleDecimalPoints => write!(f, "multiple decimal points"),
            DecimalError::NoDigits => write!(f, "input contains no digits"),
            DecimalError::DivisionByZeroOrTooSmall => write!(f, "division by zero or too small"),
        }
    }
}
...
///
/// Invariants:
/// - `digits` is a big-endian base-10 vector (most significant digit first).
/// - Each digit is in 0..=9.
/// - No leading zeros unless the value is exactly zero.
/// - `scale` is the count of fractional digits; it can be negative only for
///   legacy division behavior (see `div`).
...
impl BigDecimal {
    /// Parse a decimal string with optional '-', ',' grouping separators, and '.'.
    pub fn parse(input: &str) -> Result<Self, DecimalError> {
        ...
        for (idx, ch) in s.chars().enumerate() {
            ...
            match ch {
                ...
                _ => return Err(DecimalError::InvalidChar(ch)),
            }
        }

        if !seen_digit {
            return Err(DecimalError::NoDigits);
        }
    ...
    /// Normalize the internal digits by removing leading zeros and fixing zero sign.
    ...
    /// Legacy "too small" divisor check. Matches the original behavior for
    /// well-formed inputs without extra leading zeros.
    ...
    /// Division using legacy precision rules.
    ///
    /// This computes floor(a / b) with `DIV_FRACTIONAL_DIGITS` decimal digits
    /// and then adjusts the scale by `self.scale - other.scale`.
    pub fn div(&self, other: &Self) -> Result<Self, DecimalError> {
        let left = self.normalized_for_ops();
        let right = other.normalized_for_ops();

        if right.is_too_small_divisor() {
            return Err(DecimalError::DivisionByZeroOrTooSmall);
        }
    ...
    /// Canonical formatter with optional grouping ("," every 3 digits in the integer part).
    pub fn to_string_with_grouping(&self, grouping: bool) -> String {
        format_canonical(self, grouping)
    }

    /// Legacy formatter matching the original C++ output for add/sub or mul/div.
